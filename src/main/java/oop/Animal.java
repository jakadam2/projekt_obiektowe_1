...
public class Animal implements IMapElement {
    ...
    private int[] genom; // przydałaby się osobna klasa na genom
    protected int energy; // nie za luźny dostęp?
    private int livedDays;
    private int activeGen;

    private Set<IPositionObserver> observers = new HashSet<>();
    private int eatenPlant;
    private int child; // dzieckiem zwierzęcia jest int?
    ...
    protected MapDirection orientation; // nie za luźny dostęp?
    ...
    public void move() {
        ...
        if (moveType == MoveType.RANDOM) { // if to nie jest najlepsze rozwiązanie
    ...
    public Animal breed(Animal partner) { // proszę przemyśleć metodę statyczną przyjmującą dwa zwierzęta
        child++;
        partner.informBreed(); // a czemu nie partner.child++?
        int motherLoseEnergy = (int) (this.energy * this.loseBreedEnergy); // raz jest this i partner, a raz father i mother - mylące
        ...
        Animal stronger; // a raz stronger i weaker - jeszcze bardziej mylące
        Animal weaker;
        ...
        if (rand == 0) {
            for (int i = 0; i < separator; i++) genome[i] = stronger.genom[i];  // https://docs.oracle.com/javase/7/docs/api/java/lang/System.html#arraycopy(java.lang.Object,%20int,%20java.lang.Object,%20int,%20int)
...
//        // mieszam tablice indeksow aby nastepnie wziac sobie pierwszą x ilość jako te wylosowane geny do mutacji
    ...
    public int getColor() {
        if (energy < 0) {
            throw new IllegalStateException("DRAWING DEAD ANIMALS IS UNAVAILABLE");
        }
