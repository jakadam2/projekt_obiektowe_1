...
public abstract class AbstractWorldMap implements IPositionObserver {
    public final int x;//włącznie 500 0 - 499 // co to jest x w przypadku mapy? położenie mapy... gdzie?
    ...
    public void place(IMapElement element) {
        if (!(element.getPosition().x <= this.x + 1 && element.getPosition().y <= this.y + 1)) {
    ...
    public void update(Vector2d oldPosition, Vector2d newPosition, Animal animal) {
        ...
        if (this.boxes.get(oldPosition).isEmpty()) {
            this.boxes.remove(oldPosition);
        }
            ...
            if (box.includeAnimal()) {
                ...
                if (animals.size() > 1) {
                    ...
                    if (animalIterator.hasNext()) {
                        ...
                        if (strongest.equals(stronger)) {
                            stronger = animalIterator.next();
                        }
                        for (Animal animal : animals) {
                            if (animal.getEnergy() > stronger.getEnergy() && !animal.equals(stronger)) {
                                stronger = animal;
                            }
                        }
    ...
    public void addGrass() {
        if (plantType == PlantType.JUNGLE) { // if to nie jest najlepsze rozwiązanie
